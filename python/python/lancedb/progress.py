...
class TqdmProgressAdapter:
    ...
    def __call__(self, progress: WriteProgress) -> None:
        ...
        # Update postfix with additional stats if available
        postfix = {}
        if self.show_bytes:
...
def make_progress_callback(
    ...
    ...     # callback is now a TqdmProgressAdapter

    >>> def my_callback(progress):
    ...     print(progress["rows_written"])
    >>> callback = make_progress_callback(my_callback)
    >>> # callback is my_callback unchanged
    """
    if reporter is None:
        return None

    # Check if it's already a raw callback (callable but not a tqdm-like object)
